...
> **目录会话**：Sublime Text 的 `-w` 选项只对*文件*生效，所以单独的 `subl -w dir/` 会立即返回而窗口仍开着。为了获得可靠的关闭信号，Lend 会在文件夹旁打开一个空的 `.lend-wait` 哨兵文件；关闭窗口时哨兵也会关闭，从而解除 `-w` 的阻塞并立即结束会话（删除临时副本）。等待选项本来就阻塞文件夹的编辑器（VS Code `code -w dir/`）无需哨兵，会在窗口关闭时立即结束。你可能会在编辑器里看到一个多余的空白 `.lend-wait` 标签页——不要动它，正是它把会话生命周期与窗口绑定在一起。
