...
> **目录会话**：Sublime Text 的 `-w` 选项只对*文件*生效，所以 `subl -w dir/` 会立即返回而窗口仍开着。因此对目录参数，Lend **不会**把编辑器退出当作关闭信号——它会保留临时副本并继续在每次保存时同步，只在 **15 分钟无保存**（无活动）或 SSH 连接断开后才结束会话。对于等待选项也能阻塞目录窗口的编辑器（VS Code `code -w dir/`），则会在窗口关闭时立即结束。
