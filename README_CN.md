...
> **目录会话**：Sublime Text 的 `-w` 选项只对*文件*生效，所以单独的 `subl -w dir/` 会立即返回而窗口仍开着。为了干净地检测窗口关闭，Lend 会把一个小型窗口监听插件（`lend.py`）安装到 Sublime 的 `Packages/User` 目录；它会报告所有窗口打开的文件夹，Lend 在对应窗口关闭的那一刻立即结束目录会话。安装或升级 Lend 后请**重启一次 Sublime**让插件加载——在此之前，Lend 会退回使用一个空的 `.lend-wait` 标签页来把会话与窗口绑定（不要动它）。VS Code（`code -w dir/`）本身就能阻塞目录窗口，无需插件。
