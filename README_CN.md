...
- **退出 SSH 登录时**，会话结束，本地临时副本被删除（同步进程与你的会话绑定）。
- 远程 shell 永不阻塞；同步在一个后台进程中运行。

已自动识别 `subl`/`sublime`/`sublime_text`、`code`/`code-insiders`、`atom`、`gedit` 和 `kate`。只有打开文件或目录的调用才会变成实时同步会话——裸的 `subl`、`subl -h` 或 `code --version` 会同步执行并把输出返回远程。其它编辑器请手动传入等待选项（如 `some-editor --wait file.txt`）。`vim` 等终端编辑器需要 TTY，不适合后台守护进程模式。

打开目录时，会先把整棵目录树（含隐藏文件、嵌套子目录和符号链接）物化到本地，因此编辑器能读到目录下所有文件。

**目录会话如何结束**（与编辑器无关——无需插件、无需额外标签页）：Lend 会以等待选项启动编辑器。对于文件，以及那些等待选项对文件夹同样生效的编辑器（如 `code -w dir/`），进程退出即是关闭信号，窗口一关闭会话就立即结束。对于等待选项对文件夹会立即返回的编辑器（如 Sublime Text 的 `subl -w dir/`），Lend 会在短暂宽限期后识别出这种情况，并继续保留临时副本、每次保存都同步，直到**一段时间内没有任何保存**才结束（默认 2 分钟；可通过 `LEND_DIR_IDLE_TIMEOUT` 设置，如 `5m`/`1h`）。会话结束时，临时副本被删除、同步连接同时关闭；如果同步连接本身中断（远端进程被杀或隧道断开），会话也会立即结束。
